...
class ClockworkRNN(object):
    ...
    def _build_model(self):
        ...
        activation_hidden = tf.tanh  # try tf.nn.softplus or maxout(write)
        activation_output = tf.nn.relu

        # Split into list of tensors, one for each timestep, [num_train] * num_step
        x_list = [tf.squeeze(x, axis=[1])
                  for x in tf.split(
                    axis=1, num_or_size_splits=self.config.num_steps,
                    value=self.inputs, name="inputs_list")]

        # Periods of each group: 1,2,4, ..., 256
        ...
        with tf.variable_scope("clockwork_cell") as scope:
            ...
            for time_step in range(self.config.num_steps):
                ...
                # Compute (W_I*x_t + b_I) group_index
                ...
                # Copy the updates to the cell state, only update group_index small cells
            ...
            #self.predictions = activation_output(self.predictions, name="output") # here has # before
    ...
    def _init_optimizer(self):
        ...
        self.learning_rate = tf.maximum(self.learning_rate, self.config.learning_rate_min) #learningrate shouldn't be very small
        ...
        elif self.config.optimizer == 'rmsprop': #best!!!
            # RMSProper optimizer
        ...
        # For gradient clipping to prevent gradient explosion and vanishing.
