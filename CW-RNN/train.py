...
from clockwork_rnn import ClockworkRNN
...
def train(config):
    ...
    # steps_per_epoch is to complete one epoch, take how many steps
    step_in_epoch, steps_per_epoch = 0, int(math.floor(len(X_train)/config.batch_size))
    num_steps = steps_per_epoch*config.num_epochs # num_steps is total steps to finish all epochs
    train_step = 0
    ...
    tf_vars_to_save = tf.trainable_variables() + [model.global_step] # save all trainable variable and global_step
    ...
    for _ in range(num_steps):
        ...
        _, train_step, train_loss, learning_rate, train_summary ,train_predictions= sess.run(
            [model.train_op,
             model.global_step,
             model.loss,
             model.learning_rate,
        ...
        pred_train.append(train_predictions)
        ...
        if step_in_epoch == steps_per_epoch: # When a epoch finish

            np.save('./pred_training/predict_y_%04i' % (train_step), pred_train)
            pred_train = [] # done training a epoch
            all_pred = []
            ...
            for validation_step in range(int(math.floor(num_validation/config.batch_size))):
                ...
                if validation_step == 0:
                    ...
                    plt.draw()
                    plt.pause(0.001)
                    plt.savefig("./graph/GroundTruthandPredictions_%04i.jpg" % (train_step))

                np.save('./pred_validation/predict_y_%04i'%(train_step),all_pred)
            ...
            '''
            # Shuffle training data #### NO shuffle, because I want the raw series
...
if __name__ == "__main__":
    train(Config())
    cw_train_all = np.load('./pred_training/predict_y_4320.npy')
    cw_train_all = cw_train_all.reshape([np.shape(cw_train_all)[0] * np.shape(cw_train_all)[1], 1])

    raw_data = np.loadtxt("/Users/XuLiu/Documents/cwrnn/price_avg.txt")
    raw_data = np.reshape(raw_data, [np.shape(raw_data)[0], 1])

    loss_train = math.sqrt(mean_squared_error(raw_data[124:-550], cw_train_all))  # 0.515395518873957 #0.5602648938139334
    print loss_train

    cw_pred = np.load('./pred_validation/predict_y_4320.npy')
    cw_pred_all = cw_pred.reshape([np.shape(cw_pred)[0] * np.shape(cw_pred)[1], 1])

    loss_validation = math.sqrt(mean_squared_error(raw_data[-550:], cw_pred_all))
