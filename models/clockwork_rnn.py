...
class ClockworkRNN(object):
    ...
    def _build_model(self):
        ...
        initializer_weights = tf.contrib.layers.variance_scaling_initializer()  #随机数初始化
        initializer_bias    = tf.constant_initializer(0.0) #0初始化

        # Activation functions of the hidden and output state
        activation_hidden = tf.tanh  #此处可以改为tf.nn.softplus试试
        ...
        # Mask for matrix W_I to make sure it's upper triangular 上三角都为1
        ...
        with tf.variable_scope("clockwork_cell") as scope:
            ...
            for time_step in range(self.config.num_steps):
                ...
                # Compute (W_I*x_t + b_I) 用group_index去定位到激活的那个组之前的所有组，去切片
    ...
    def _init_optimizer(self):
        ...
        # Learning rate decay, note that is self.learning_rate_decay == 1.0,学习率在不断下降
        ...
        self.learning_rate = tf.maximum(self.learning_rate, self.config.learning_rate_min) #learningrate 不能太小
